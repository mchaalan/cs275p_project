...
from dynamic_metrics import DynamicKID
...
class DiffusionModel(keras.Model):
    def __init__(self, img_size=64, patch_size=4, embed_dim=256, depth=6, num_heads=6):
        super().__init__()
        self.normalizer = layers.Normalization()
        self.network = dit_model(img_size, patch_size, embed_dim, depth, num_heads)
    ...
    """
    [Kept baseline implementation]
    Explanation: This is controlling the rate of denoising and noising. From the 
    keras explanation and DDIM paper, they stated how "we need to have a function
    that tells us at each point in the diffusion process the noise levels and signal 
    levels of the noisy image corresponding to the actual diffusion time."
    """
    ...
    def download_images(self, epoch=None, logs=None, description="img_dit_normal", num_rows=3, num_cols=6):
        generated_images = self.generate(
            num_images=num_rows * num_cols,
            diffusion_steps=plot_diffusion_steps,
        )
        ...
        for row in range(num_rows):
            for col in range(num_cols):
                index = row * num_cols + col
                save_image(generated_images[index], f"{description}/image_{col}_{row}_{index}.png")
...
if train:
    ...
    # model.download_images(description="img_dit_normal", num_rows=100, num_cols=10)
